...
public class CardImplSeaside extends CardImpl {
  ...
  private void ambassador(Game game, MoveContext context, Player currentPlayer) {
      ...
      if (returnCount > 2) {
        ...
        if (returnCount > inHandCount) {
          Util.playerError(currentPlayer,
            "Ambassador return to supply error (more than cards in hand), returning " + inHandCount);
    ...
    for (Player player : game.getPlayersInTurnOrder()) {
      if (player != currentPlayer && !Util.isDefendedFromAttack(game, player, this)) {
        ...
        if (returnCount > -1) {
          if (pile.isSupply()) {
            player.gainNewCard(card, getControlCard(), new MoveContext(game, player));
          }
        }
      }
    }
  ...
  private void embargo(Game game, MoveContext context, Player currentPlayer) {
    ...
    while (game.addEmbargo(card) == null) {
      ...
      while (true) {
        ...
        if (game.isValidEmbargoPile(card)) {
          break;
        }
      }
    }
  ...
  private void explorer(MoveContext context, Player currentPlayer) {
    ...
    if (province != null && currentPlayer.controlPlayer.explorer_shouldRevealProvince(context)) {
      currentPlayer.reveal(province, getControlCard(), context);
      treasure = Cards.gold;
    } else {
      treasure = Cards.silver;
    }

    currentPlayer.gainNewCard(treasure, getControlCard(), context);
  }
        ...
        if (player.hand.size() >= 4) {
          ...
          if (bad) {
            Util.playerError(player,
              "Ghost Ship put back cards error, putting back the first " + (player.hand.size() - 3) + " cards.");
  ...
  private void haven(MoveContext context, Player currentPlayer) {
    ...
    if (card != null) {
      ...
      context.game.broadcastEvent(event);
    } else if (getControlCard().cloneCount == 1) {
  ...
  private void navigator(Game game, MoveContext context, Player currentPlayer) {
    ...
    if (!topOfTheDeck.isEmpty()) {
      if (currentPlayer.controlPlayer
        ...
        Card[] order =
          currentPlayer.controlPlayer.navigator_cardOrder(context, topOfTheDeck.toArray(new Card[topOfTheDeck.size()]));
      ...
      for (Player targetPlayer : playersToAttack) {
        ...
        if (treasures.size() == 1) {
          ...
          if (treasures.get(0).equals(treasures.get(1))) {
            cardToTrash = treasures.get(0);
            targetPlayer.discard(treasures.get(1), getControlCard(), targetContext);
          } else {
            cardToTrash =
              currentPlayer.controlPlayer.pirateShip_treasureToTrash(context, treasures.toArray(new Card[] {}));
          }
  ...
  private void treasureMap(MoveContext context, Player currentPlayer) {
    ...
    if (numberTimesAlreadyPlayed == 0) {
      if (anotherMap != null && !getControlCard().equals(Cards.estate)) {
        // going to get the gold so trash map played and map from hand
        currentPlayer.trash(currentPlayer.playedCards.removeCard(getControlCard()), null, context);
