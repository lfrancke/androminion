...
public class CardImplCornucopia extends CardImpl {
    ...
    for (Player player : game.getPlayersInTurnOrder()) {
      if (player != currentPlayer && !Util.isDefendedFromAttack(game, player, this)) {
        ...
        if (player.hand.size() > keepCardCount) {
          ...
          player.discardRemainingCardsFromHand(context, cardsToKeep, getControlCard(), keepCardCount);
        }
      }
    }
  }
  ...
  private void hamlet(MoveContext context, Player currentPlayer) {
    ...
    if (forAction != null) {
      ...
      currentPlayer.discard(forAction, getControlCard(), context);
      context.actions++;
    }
    ...
    if (forBuy != null) {
      ...
      currentPlayer.discard(forBuy, getControlCard(), context);
      context.buys++;
    }
  }
  ...
  private void jester(Game game, MoveContext context, Player currentPlayer) {
    for (Player targetPlayer : game.getPlayersInTurnOrder()) {
      if (targetPlayer != currentPlayer && !Util.isDefendedFromAttack(game, targetPlayer, this)) {
        ...
        targetPlayer.reveal(draw, getControlCard(), targetContext);
        targetPlayer.discard(draw, getControlCard(), targetContext);
        ...
        if (draw.is(Type.Victory, targetPlayer)) {
          ...
          if (!game.isPileEmpty(draw) &&
            ...
            toGainContext.getPlayer().gainNewCard(draw, getControlCard(), toGainContext);
          }
        }
      }
    }
      ...
      if (province != null) {
        ...
        if (player == currentPlayer) {
          ...
          if (currentPlayerRevealed) {
            currentPlayer.reveal(province, getControlCard(), playerContext);
            player.hand.remove(province);
            currentPlayer.discard(province, getControlCard(), playerContext);
          }
        } else {
          if (player.controlPlayer.tournament_shouldRevealProvince(playerContext)) {
            player.reveal(province, getControlCard(), playerContext);
      ...
      for (TrustySteedOption option : options) {
        if (option == TrustySteedOption.AddActions) {
          ...
          while (currentPlayer.getDeckSize() > 0) {
            currentPlayer.discard(currentPlayer.deck.remove(0), getControlCard(), null);
          }
        }
      }
    }
  ...
  private void youngwitch(Game game, MoveContext context, Player currentPlayer) {
    ...
    for (Player targetPlayer : game.getPlayersInTurnOrder()) {
      if (targetPlayer != currentPlayer && !Util.isDefendedFromAttack(game, targetPlayer, this)) {
        if (targetPlayer.hand.contains(game.baneCard) && game.pileSize(Cards.curse) > 0 && targetPlayer
                                                                                             .revealBane(context)) {
          targetPlayer.reveal(game.baneCard, getControlCard(), new MoveContext(game, targetPlayer));
        } else {
          targetPlayer.attacked(getControlCard(), context);
          MoveContext targetContext = new MoveContext(game, targetPlayer);
          targetContext.attackedPlayer = targetPlayer;
