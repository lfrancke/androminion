...
public abstract class IndirectPlayer extends QuickPlayPlayer {
  ...
  public int selectInt(MoveContext context, Card responsible, int maxInt, Object extra) {
    ...
    if (extra != null) {
      ...
      options[0] = extra;
      System.arraycopy(int_options, 0, options, 1, int_options.length);
    }
  ...
  public Card remodel_cardToObtain(MoveContext context, int maxCost, int maxDebtCost, boolean potion) {
    ...
    SelectCardOptions sco =
      new SelectCardOptions().maxCost(maxCost).maxDebtCost(maxDebtCost).maxPotionCost(potion ? 1 : 0)
  ...
  public Card[] thief_treasuresToGain(MoveContext context, Card[] treasures) {
    ...
    options.add(null);
    Collections.addAll(options, treasures);
  ...
  public Card mine_treasureToObtain(MoveContext context, int maxCost, int maxDebtCost, boolean potion) {
    ...
    SelectCardOptions sco = new SelectCardOptions().isTreasure().maxCost(maxCost).maxDebtCost(maxDebtCost)
                              .maxPotionCost(potion ? 1 : 0).setActionType(ActionType.GAIN)
                              .setCardResponsible(Cards.mine);
    return getFromTable(context, sco);
  }
  ...
  public SentryOption sentry_chooseOption(MoveContext context, Card card, Card[] cards) {
    ...
    options[0] = card;
    options[1] = cards;
    System.arraycopy(sentryOptions, 0, options, 2, sentryOptions.length);
  ...
  public Card saboteur_cardToObtain(MoveContext context, int maxCost, int maxDebtCost, boolean potion) {
    ...
    SelectCardOptions sco = new SelectCardOptions().setPassable()
                              .maxCost(maxCost).maxDebtCost(maxDebtCost).maxPotionCost(potion ? 1 : 0)
                              .setCardResponsible(Cards.saboteur);
    return getFromTable(context, sco);
  }
  ...
  public Card lurker_cardToGainFromTrash(MoveContext context) {
    ...
    ArrayList<Card> options = new ArrayList<>(inTrashPile);
  ...
  public Card lookout_cardToTrash(MoveContext context, Card[] cards) {
    ...
    System.arraycopy(cards, 0, options, 1, cards.length);
  ...
  public boolean royalSealTravellingFair_shouldPutCardOnDeck(MoveContext context, Card responsible, Card card) {
    if (context.isQuickPlay() && shouldAutoPlay_royalSealTravellingFair_shouldPutCardOnDeck(context, responsible,
      card)) {
  ...
  public WatchTowerOption watchTower_chooseOption(MoveContext context, Card card) {
    ...
    options[0] = card;
    System.arraycopy(watchTower_options, 0, options, 1, watchTower_options.length);
  ...
  public ArrayList<Card> treasureCardsToPlayInOrder(MoveContext context, int maxCards, Card responsible) {
    ...
    ArrayList<Card> treasures = new ArrayList<>();
    Collections.addAll(treasures, cards);
    return treasures;
  }
  ...
  public JesterOption jester_chooseOption(MoveContext context, Player targetPlayer, Card card) {
    ...
    options[1] = card;
    System.arraycopy(jester_options, 0, options, 2, jester_options.length);
  ...
  public Card[] cartographer_cardsFromTopOfDeckToDiscard(MoveContext context, Card[] cards) {
    ...
    ArrayList<Card> options = new ArrayList<>();
    options.add(null);
    Collections.addAll(options, cards);
    ...
    while (options.size() > 1) {
      ...
      cardsToDiscard.add(options.get(o));
      options.remove(o);
    }
  ...
  public Card scheme_actionToPutOnTopOfDeck(MoveContext context, Card[] actions) {
    ...
    ArrayList<Card> options = new ArrayList<>();
    Collections.addAll(options, actions);
    options.add(null);
  ...
  public Card getAttackReaction(MoveContext context, Card responsible, boolean defended, Card lastCard) {
    ...
    Collections.addAll(reactionCards, getAttackReactionCards(defended));
  ...
  public DoctorOverpayOption doctor_chooseOption(MoveContext context, Card card) {
    ...
    options[0] = card;
    System.arraycopy(doctor_options, 0, options, 1, doctor_options.length);
  ...
  public Card call_whenGainCardToCall(MoveContext context, Card gainedCard, Card[] possibleCards) {
    ...
    System.arraycopy(possibleCards, 0, options, 1, possibleCards.length);
  ...
  public Card call_whenActionResolveCardToCall(MoveContext context, Card resolvedAction, Card[] possibleCards) {
    if (context.isQuickPlay() && shouldAutoPlay_call_whenActionResolveCardToCall(context, resolvedAction,
      possibleCards)) {
  ...
  public int numDebtTokensToPayOff(MoveContext context) {
    ...
    return selectInt(context, null, Math.min(context.getCoins(), context.getPlayer().getDebtTokenCount()),
      OPTION_PAY_DEBT);
  }
  ...
  public Card charm_cardToObtain(MoveContext context, Card boughtCard) {
    SelectCardOptions sco = new SelectCardOptions()
                              .exactCost(boughtCard.getCost(context), boughtCard.getDebtCost(context),
                                boughtCard.costPotion() ? 1 : 0)
                              .not(boughtCard).setPassable().setActionType(ActionType.GAIN)
                              .setCardResponsible(Cards.charm);
    return getFromTable(context, sco);
  }
  ...
  protected abstract boolean selectBoolean(MoveContext context, Card cardResponsible, Object[] extras);

  protected abstract int selectOption(MoveContext context, Card card, Object[] options);

  protected abstract int[] orderCards(MoveContext context, int[] cards);

  protected abstract Card[] pickCards(MoveContext context, SelectCardOptions sco, int count, boolean exact);
