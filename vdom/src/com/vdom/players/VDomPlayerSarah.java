...
public class VDomPlayerSarah extends BasePlayer {
  ...
  public void setupGameVariables(GameType gameType, Card[] cardsInPlay) {
    ...
    valuedCards =
      new Card[] {Cards.torturer, Cards.bazaar, Cards.masquerade, Cards.ghostShip, Cards.wharf, Cards.smithy,
        Cards.harem, Cards.adventurer,
        Cards.shantyTown, Cards.festival, Cards.moneyLender, Cards.venture, Cards.tournament, Cards.miningVillage,
        Cards.mint, Cards.farmingVillage,
        Cards.kingsCourt, Cards.jester, Cards.youngWitch, Cards.goons, Cards.monument, Cards.bishop, Cards.hamlet,
        Cards.fortuneTeller, Cards.watchTower,
    ...
    earlyCardBuys =
      new Card[] {Cards.militia, Cards.seaHag, Cards.familiar, Cards.youngWitch, Cards.thief, Cards.pirateShip,
        Cards.rabble, Cards.goons,
    ...
    if (gameType == GameType.FirstGame || gameType == GameType.RandomBaseGame || gameType == GameType.VictoryDance
        || gameType == GameType.HandMadness
    ...
    if (gameType == GameType.BigMoney || gameType == GameType.VillageSquare || gameType == GameType.SecretSchemes
        || numCornucopia >= 5) {
    ...
    if (gameType == GameType.FirstGame) {
      ...
      midGame = 18;
      valuedCards =
        new Card[] {Cards.treasury, Cards.pearlDiver, Cards.ghostShip, Cards.seaHag, Cards.platinum, Cards.gold,
          Cards.silver};
      ...
      actionCardMax = 16;
      valuedCards =
        new Card[] {Cards.warehouse, Cards.wharf, Cards.bazaar, Cards.pearlDiver, Cards.ghostShip, Cards.platinum,
          Cards.gold,
          Cards.silver};
      ...
      valuedCards = new Card[] {Cards.warehouse, Cards.treasureMap, Cards.wharf, Cards.pearlDiver, Cards.fishingVillage,
        Cards.platinum,
      ...
      actionCardMax = 5;
      valuedCards =
        new Card[] {Cards.laboratory, Cards.adventurer, Cards.venture, Cards.royalSeal, Cards.mint, Cards.platinum,
          Cards.gold, Cards.silver};
      ...
      actionCardMax = 6;
      valuedCards =
        new Card[] {Cards.countingHouse, Cards.contraband, Cards.bureaucrat, Cards.chancellor, Cards.mountebank,
          Cards.platinum, Cards.gold, Cards.silver, Cards.copper};
      ...
      actionCardMax = 5;
      valuedCards =
        new Card[] {Cards.jester, Cards.chancellor, Cards.farmingVillage, Cards.market, Cards.baker, Cards.platinum,
          Cards.gold, Cards.silver};
      ...
      actionCardMax = 5;
      valuedCards =
        new Card[] {Cards.jester, Cards.wishingWell, Cards.miningVillage, Cards.platinum, Cards.gold, Cards.silver};
      ...
      actionCardMax = 6;
      valuedCards =
        new Card[] {Cards.tournament, Cards.jester, Cards.youngWitch, Cards.hamlet, Cards.fortuneTeller, Cards.platinum,
          Cards.gold, Cards.silver};
    } else {
      improvise = true;
      actionCardMax = 5;
  ...
  public boolean shouldPassOnBuy(MoveContext context, Card card) {
    return
      ...
      !favorSilverGoldPlat && (card.equals(Cards.silver) || card.equals(Cards.masterpiece) || card.equals(Cards.gold)
  ...
  public Card doBuy(MoveContext context) {
    ...
    if (turnCount > midGame && coinAvailableForBuy <= 7 && rand.nextInt(3) == 0) {
      if (context.canBuy(Cards.vineyard) && actionCardCount >= 9
          && context.getEmbargosIfCursesLeft(Cards.vineyard) == 0) {
        return Cards.vineyard;
      }
      if (context.canBuy(Cards.duchy) && context.getEmbargosIfCursesLeft(Cards.duchy) == 0) {
        return Cards.duchy;
      }
      if (context.canBuy(Cards.vineyard) && actionCardCount >= 6
          && context.getEmbargosIfCursesLeft(Cards.vineyard) == 0) {
        return Cards.vineyard;
      }
    }
    ...
    if (context.canBuy(Cards.grandMarket) && rand.nextInt(MAX_OF_ONE_ACTION_CARD * 2) < getMyCardCount(
      Cards.grandMarket)) {
      ...
      for (Card card : POTION_CARDS) {
        if (context.canBuy(card)) {
          if (getMyCardCount(card) >= 2
              && !(card.equals(Cards.alchemist) || card.equals(Cards.philosophersStone) || card.equals(
            Cards.scryingPool))) {
            continue;
          }
    ...
    if (favorSilverGoldPlat) {
      ...
      if (context.canBuy(Cards.gold) && game.pileSize(Cards.gold) > 0 && coinAvailableForBuy >= 6
          && coinAvailableForBuy <= 6 && rand.nextFloat() > silverLine) {
