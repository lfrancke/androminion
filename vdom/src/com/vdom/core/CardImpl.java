...
public class CardImpl implements Card, Comparable<Card> {
  ...
  public void play(Game game, MoveContext context, boolean fromHand, boolean treasurePlay) {
    ...
    boolean enchantressEffect =
      isAction && !context.enchantressAlreadyAffected && game.enchantressAttacks(currentPlayer);
    ...
    if (!actionCards.isEmpty()) {
      ...
      if (cardToPlay != null) {
        if (actionCards.contains(cardToPlay)) {
          ...
          if (cardToPlay.is(Type.Knight, currentPlayer) && !currentPlayer.playedCards.contains(cardToPlay)
              && game.trashPile.contains(cardToPlay)) {
        ...
        if (kind == Cards.Kind.Procession) {
          ...
          Card cardToGain = currentPlayer.controlPlayer.procession_cardToGain(context, 1 + cardToPlay.getCost(context),
            cardToPlay.getDebtCost(context), cardToPlay.costPotion());
  ...
  protected void discardMultiple(MoveContext context, Player currentPlayer, int numToDiscard) {
    ...
    if (hand.size() <= numToDiscard) {
      ...
      if (cardsToDiscard == null || cardsToDiscard.length != numToDiscard || !Util.areCardsInHand(cardsToDiscard,
        context)) {
