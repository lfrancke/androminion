...
public class CardImplProsperity extends CardImpl {
  ...
  private void countingHouse(MoveContext context, Player currentPlayer) {
    if (!currentPlayer.discard.isEmpty()) {
      ...
      if (coppersTotal < 0 || coppersTotal > coppers) {
        Util
          .playerError(currentPlayer, "CountingHouse error, invalid number of coppers. Taking all coppers into hand.");
  ...
  private void forge(MoveContext context, Player currentPlayer) {
    ...
    if (card != null) {
      if (card.getCost(context) != totalCost || card.getDebtCost(context) > 0 || card.costPotion() || !Cards
                                                                                                         .isSupplyCard(
                                                                                                           card)) {
    ...
    if (!currentPlayer.hand.isEmpty()) {
      ...
      if (card == null || !currentPlayer.hand.contains(card)) {
        Util.playerError(currentPlayer,
          "Trade Route card selection error, no card selected or card not in hand, choosing random card to trash");
        card = Util.randomCard(currentPlayer.hand);
      }
