...
public class CardImplHinterlands extends CardImpl {
        ...
        if (!player.getHand().isEmpty()) {
          ...
          if (cost == -1) {
            ...
            for (Card c : context.getCardsInGame(GetCardsInGameOptions.TopOfPiles, true)) {
              if (Cards.isSupplyCard(c) && c.getCost(context) == cost && c.costPotion() == potion && context
                                                                                                       .isCardOnTop(
                                                                                                         c)) {
                validCard = true;
                break;
    ...
    if (!topOfTheDeck.isEmpty()) {
      ...
      if (cardsToDiscard != null) {
        for (Card toDiscard : cardsToDiscard) {
          if (topOfTheDeck.remove(toDiscard)) {
            currentPlayer.discard(toDiscard, getControlCard(), null);
          } else {
      ...
      if (!topOfTheDeck.isEmpty()) {
        ...
        if (topOfTheDeck.size() == 1) {
          ...
          order = currentPlayer.controlPlayer
                    .cartographer_cardOrder(context, topOfTheDeck.toArray(new Card[topOfTheDeck.size()]));
  ...
  private void crossroads(Game game, MoveContext context, Player currentPlayer) {
    ...
    if (!getControlCard().equals(Cards.estate)) {
      int crossroadsPlayed = getControlCard().numberTimesAlreadyPlayed;
    ...
    if (!currentPlayer.hand.isEmpty()) {
      ...
      if (context.isNewCardAvailable(trashedCardCost - 1, trashedCardDebt, trashedCardPotion)) {
        lowCardToGain = currentPlayer.controlPlayer
                          .develop_lowCardToGain(context, trashedCardCost - 1, trashedCardDebt, trashedCardPotion);
      ...
      if (cardsToGain.length == 0) {
        for (Card c : context.getCardsInGame(GetCardsInGameOptions.TopOfPiles, true)) {
          if (Cards.isSupplyCard(c) && (c.getCost(context) == trashedCardCost - 1
                                        || c.getCost(context) == trashedCardCost + 1) && context.isCardOnTop(c)) {
            bad = true;
          }
        }
      } else if (cardsToGain.length == 1) {
        if (cardsToGain[0].getCost(context) != trashedCardCost - 1
            && cardsToGain[0].getCost(context) != trashedCardCost + 1) {
          bad = true;
        } else {
          int costToCheck;
          if (cardsToGain[0].getCost(context) == trashedCardCost - 1) {
        ...
        Card lowCard =
          (cardsToGain[0].getCost(context) <= cardsToGain[1].getCost(context)) ? cardsToGain[0] : cardsToGain[1];
        Card highCard =
          (cardsToGain[0].getCost(context) > cardsToGain[1].getCost(context)) ? cardsToGain[0] : cardsToGain[1];
  ...
  private void duchess(Game game, MoveContext context) {
    for (Player targetPlayer : game.getPlayersInTurnOrder()) {
      ...
      if (c != null) {
        ...
        if (discard) {
          targetPlayer.discard(c, getControlCard(), targetPlayerContext);
        } else {
  ...
  private void embassy(MoveContext context, Player currentPlayer) {
    ...
    for (Card card : cards) {
      ...
      currentPlayer.discard(card, getControlCard(), null);
    }
  }
  ...
  private void mandarin(MoveContext context, Player currentPlayer) {
    if (!currentPlayer.hand.isEmpty()) {
      ...
      currentPlayer.reveal(toTopOfDeck, getControlCard(), context);
  ...
  private void margrave(Game game, MoveContext context, Player currentPlayer) {
    for (Player player : game.getPlayersInTurnOrder()) {
      if (player != currentPlayer && !Util.isDefendedFromAttack(game, player, this)) {
        ...
        game.drawToHand(playerContext, getControlCard(), 1);
      ...
      if (targetPlayer != player && (!defensible || !Util.isDefendedFromAttack(context.game, targetPlayer, this))) {
        ...
        if (silverOrGold.size() == 1) {
          ...
          if (silverOrGold.get(0).equals(silverOrGold.get(1))) {
            cardToTrash = silverOrGold.get(0);
            targetPlayer.discard(silverOrGold.get(1), getControlCard(), targetContext);
          } else {
            cardToTrash =
              (player).controlPlayer.nobleBrigand_silverOrGoldToTrash(moveContext, silverOrGold.toArray(new Card[] {}));
  ...
  private void oasis(MoveContext context, Player currentPlayer) {
    if (!currentPlayer.hand.isEmpty()) {
      ...
      currentPlayer.reveal(cardToDiscard, getControlCard(), context);
      currentPlayer.discard(cardToDiscard, getControlCard(), null);
    }
  }
    ...
    for (Player targetPlayer : game.getPlayersInTurnOrder()) {
      if (targetPlayer == currentPlayer || !Util.isDefendedFromAttack(game, targetPlayer, this)) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          targetPlayer.reveal(c, getControlCard(), targetContext);
          cards.add(c);
        }
        ...
        if (!cards.isEmpty()) {
          if (currentPlayer.controlPlayer.oracle_shouldDiscard(context, targetPlayer, cards)) {
            ...
            Card[] order =
              (targetPlayer).controlPlayer.oracle_orderCards(context, cards.toArray(new Card[cards.size()]));
            boolean bad = false;
    ...
    if (handContainsTreasure) {
      ...
      if (treasure != null) {
        if (!currentPlayer.hand.contains(treasure) || !treasure.is(Type.Treasure, currentPlayer)) {
          ...
          currentPlayer.trash(treasure, getControlCard(), context);
  ...
  private void trader(MoveContext context, Player currentPlayer) {
    if (!currentPlayer.hand.isEmpty()) {
      ...
      for (int i = 0; i < cost; i++) {
        if (currentPlayer.gainNewCard(Cards.silver, getControlCard(), context) == null) {
          break;
        }
      }
    }
