...
public class Game {
  ...
  protected static void processNewGameArgs(String[] args) throws ExitException {
    ...
    if (numPlayers < 1 || numPlayers > 6 || showUsage) {
      Util.log(
        "Usage: [-debug][-ignore(playername)][-count(# of Games)][-type(Game type)] class1 class2 [class3] [class4]");
      throw new ExitException();
    }
    ...
    for (String arg : args) {
      ...
      if (arg.startsWith("-")) {
        if (arg.toLowerCase().equals(quickPlayArg)) {
          ...
          blackMarketSplitPileOptions =
            BlackMarketSplitPileOptions.valueOf(arg.substring(bmSplitPileArg.length()).toUpperCase());
  ...
  protected void playGuildsTokens(Player player, MoveContext context) {
    ...
    if (coinTokenTotal > 0) {
      ...
      int numTokensToSpend =
        player.controlPlayer.numGuildsCoinTokensToSpend(context, coinTokenTotal, false/*!butcher*/);
  ...
  protected void playerBuy(Player player, MoveContext context) {
    ...
    if (context.getCoinAvailableForBuy() >= 2) {
      ...
      if (wineMerchants > 0) {
        ...
        if (wineMerchants < 0 || wineMerchantsTotal > wineMerchants) {
          Util.playerError(player,
            "Wine Merchant discard error, invalid number of Wine Merchants. Discarding all Wine Merchants.");
        ...
        if (wineMerchantsTotal > 0) {
          for (int i = 0; i < wineMerchantsTotal; i++) {
            ...
            player.discard(card, null, context, true,
              false); //set commandedDiscard=true and cleanup=false to force GameEvent
          }
        }
      }
    }
    ...
    if (context.getCoinAvailableForBuy() >= 2) {
      ...
      if (wineMerchants > 0) {
        ...
        if (wineMerchants < 0 || wineMerchantsTotal > wineMerchants) {
          Util.playerError(player,
            "Wine Merchant estate discard error, invalid number of Wine Merchants. Discarding all Wine Merchants.");
          wineMerchantsTotal = wineMerchants;
        }
        if (wineMerchantsTotal > 0) {
          for (int i = 0; i < wineMerchantsTotal; i++) {
  ...
  protected void playerBeginTurn(Player player, MoveContext context) {
    ...
    while (durationEffects.size() > numOptionalItems) {
      ...
      if (allDurationAreSimple) {
        ...
        selection = 2 * player.controlPlayer
                          .duration_cardToPlay(context, durationEffects.toArray(new Object[durationEffects.size()]));
      }
  ...
  protected void initPlayers(int numPlayers, boolean isRandom) throws ExitException {
    ...
    for (int i = 0; i < numPlayers; i++) {
      ...
      if (platColonyPassedIn || chanceForPlatColony > 0.9999) {
        ...
        s += "Chance for Platinum/Colony\n   " + (Math.round(chanceForPlatColony * 100)) + "% ... " + (isPlatInGame()
                                                                                                         ? "included\n"
                                                                                                         : "not included\n");
      }
  ...
  protected void initCards() {
    ...
    if (cardsSpecifiedAtLaunch != null) {
      ...
      CardSet cardSet = CardSet
                          .getCardSet(gameType, -1, randomExpansions, randomExcludedExpansions, randomIncludesEvents,
                            numRandomEvents, randomIncludesLandmarks, numRandomLandmarks, !splitMaxEventsAndLandmarks,
                            true);
      if (cardSet == null) {
    ...
    if (piles.containsKey(Cards.blackMarket.getName())) {
      ...
      if (blackMarketOnlyCardsFromUsedExpansions) {
        ...
        allCards = CardSet
                     .getCardSet(GameType.Random, count + 10, expansions, randomExcludedExpansions, false, 0, false, 0,
                       false, false).getCards();
      } else {
      ...
      if (blackMarketSplitPileOptions == BlackMarketSplitPileOptions.ALL) {
        ...
        for (int i = 0; i < cards.size(); ++i) {
          ...
          ArrayList<Card> templates = c.getPileCreator().create(allCards.get(i), 12)
                                        .getTemplateCards(); //count doesn't matter as we only need templates
          if (templates.size() > 1) {
    ...
    Card[] landmarksWithTokens =
      {Cards.arena, Cards.basilica, Cards.battlefield, Cards.baths, Cards.colonnade, Cards.labyrinth};
    ...
    for (Card c : cards) {
      cardListText +=
        Util.getShortText(c) + ((baneCard != null && c.equals(baneCard)) ? " (Bane)" + baneCard.getName() : "") + "\n";
    }
    if (!events.isEmpty()) {
      cardListText += "\nEvents in play\n---------------\n";
      for (Card c : events) {
    ...
    if (!landmarks.isEmpty()) {
      ...
      for (Card c : landmarks) {
        cardListText +=
          c.getName() + (c.equals(Cards.obelisk) && obeliskCard != null ? " (" + obeliskCard.getName() + ")" : "")
          + "\n";
      }
    }
      ...
      public void gameEvent(GameEvent event) {
        ...
        if ((event.getType() == GameEvent.EventType.CardObtained || event.getType() == GameEvent.EventType.BuyingCard)
          ...
          if (event.card.equals(Cards.estate) && event.player.getInheritance() != null) {
            ((CardImpl) event.card)
              .startInheritingCardAbilities(player.getInheritance().getTemplateCard().instantiate());
          }
          ...
          if (!event.newCard) {
            boolean hasInheritedTrader =
              Cards.trader.equals(context.getPlayer().getInheritance()) && context.getPlayer().hand
          ...
          boolean hasInheritedWatchtower =
          ...
          if (!handled) {
            if (context.isRoyalSealInPlay() && context.player.controlPlayer
              ...
              player.putOnTopOfDeck(event.card, context, true);
                       ...
                       && (!event.card.equals(Cards.borderVillage) || (event.card.equals(Cards.borderVillage)
                                                                       && Cards.borderVillage.getCost(context) == 0))
                       && (!event.card.equals(Cards.deathCart) || (event.card.equals(Cards.deathCart) && context.game
                                                                                                           .isPileEmpty(
                                                                                                             Cards.virtualRuins)))
                     ) {
              ...
              if (r.equals(Cards.armory)
                  ...
                  || r.equals(Cards.replace) && context.attackedPlayer != player && (gainedCardAbility.is(Type.Action)
                                                                                     || gainedCardAbility
                                                                                          .is(Type.Treasure))) {
                player.putOnTopOfDeck(event.card, context, true);
              ...
              } else if (r.equals(Cards.tradingPost) || r.equals(Cards.mine) || r.equals(Cards.explorer) || r.equals(
                Cards.torturer) || r.equals(Cards.transmogrify) || r.equals(Cards.artisan)) {
                player.hand.add(event.card);
          ...
          for (Card c : player.tavern) {
            if (c.behaveAsCard().isCallableWhenCardGained()) {
              ...
              if (callCost == -1 || (event.card.getCost(context) <= callCost && event.card.getDebtCost(context) == 0
                                     && !event.card.costPotion())) {
                callableCards.add(c);
              }
            }
          }
          ...
          if (gainedCardAbility.equals(Cards.illGottenGains)) {
            ...
            for (Card c : event.context.getCardsInGame(GetCardsInGameOptions.TopOfPiles, true)) {
              if (Cards.isSupplyCard(c) && c.getCost(context) < gainedCardCost && !c.costPotion()
                  && c.getDebtCost(context) <= 0 && event.context.isCardOnTop(c)) {
                validCard = true;
                break;
              }
            }
            ...
            if (!treasureCardsInPlay.isEmpty()) {
              ...
              if (treasureCardsInPlay.size() == 1) {
                ...
                order = player.controlPlayer.mandarin_orderCards(context,
                  treasureCardsInPlay.toArray(new Card[treasureCardsInPlay.size()]));
              }
            ...
            for (Player targetPlayer : context.game.getPlayersInTurnOrder()) {
              ...
              if (targetPlayer.hand.size() >= 5) {
                ...
                if (bad) {
                  Util
                    .playerError(targetPlayer, "Haunted Castle put back cards error, putting back the first 2 cards.");
                  cards = new Card[2];
            ...
            if (option == null) {
              option = HuntingGroundsOption.GainEstates;
            }
            switch (option) {
              case GainDuchy:
        ...
        if (!shouldShow) {
          if (event.getType() != GameEvent.EventType.TurnBegin && event.getType() != GameEvent.EventType.TurnEnd
              && event.getType() != GameEvent.EventType.DeckReplenished
        ...
        if (!showEvents.contains(event.getType()) && shouldShow) {
          ...
          if (event.getType() == GameEvent.EventType.BuyingCard) {
            msg.append(
              " (with gold: " + event.getContext().getCoinAvailableForBuy() + ", buys remaining: " + event.getContext()
                                                                                                       .getBuysLeft());
          }
  ...
  protected Card takeFromPileCheckTrader(Card cardToGain, MoveContext context) {
    ...
    boolean hasInheritedTrader =
      Cards.trader.equals(context.getPlayer().getInheritance()) && context.getPlayer().hand.contains(Cards.estate);
    boolean hasTrader = context.getPlayer().hand.contains(Cards.trader);
    Card traderCard = hasTrader ? Cards.trader : Cards.estate;
    ...
    if (test) {
      ...
      for (CardPile pile : piles.values()) {
        ...
        if (!card.equals(Cards.copper) && !card.equals(Cards.silver) && !card.equals(Cards.gold) && !card.equals(
          Cards.platinum)
            && !card.equals(Cards.estate) && !card.equals(Cards.duchy) && !card.equals(Cards.province) && !card.equals(
          Cards.colony)
  ...
  Card playBuy(MoveContext context, Card buy) {
    ...
    if (!buy.costPotion() && buy.getDebtCost(context) == 0 && !(buy.is(Type.Victory)) && cost < 5 && !buy.is(
      Type.Event)) {
  ...
  private void handleShowEvent(GameEvent event) {
    if (showEvents.contains(event.getType())) {
      ...
      if (player == null || (player != null && !showPlayers.isEmpty() && !showPlayers
