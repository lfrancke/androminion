...
public class CardImplAlchemy extends CardImpl {
  ...
  private void golem(Game game, MoveContext context, Player currentPlayer) {
    ...
    if (!toOrder.isEmpty()) {
      ...
      if (toOrder.size() == 1) {
        ...
        Card[] playOrder =
          currentPlayer.controlPlayer.golem_cardOrder(context, toOrder.toArray(new Card[toOrder.size()]));
  ...
  private void transmute(MoveContext context, Player currentPlayer) {
    ...
    if (cardToTrash == null) {
      ...
      if (cardToTrash.is(Type.Treasure)) {
        currentPlayer.gainNewCard(Cards.transmute, getControlCard(), context);
      }
  ...
  private void university(MoveContext context, Player currentPlayer) {
    ...
    if (cardToObtain != null && cardToObtain.is(Type.Action, null) && cardToObtain.getCost(context) <= 5
        && !cardToObtain.costPotion()) {
      currentPlayer.gainNewCard(cardToObtain, getControlCard(), context);
    }
  }
}
