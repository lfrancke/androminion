...
public class Comms {
  ...
  public Comms(EventHandler parent, int port) throws IOException {
    ...
    if (networkThread.thrownIOException != null) {
      throw networkThread.thrownIOException;
    }
  }
  ...
  public Comms(EventHandler parent, String host, int port)
    ...
    if (networkThread.thrownUnknownHostException != null) {
      throw networkThread.thrownUnknownHostException;
    }
    if (networkThread.thrownStreamCorruptedException != null) {
      throw networkThread.thrownStreamCorruptedException;
    }
    if (networkThread.thrownIOException != null) {
      throw networkThread.thrownIOException;
    }
  ...
  public Event doWaitTimeout(long timeout) {
    ...
    if (usetimeout) {
      endtime = System.nanoTime()
                + timeout * 1000 * 1000; // Don't use System.currentTimeMillis, it fails on timezone change etc.
    }
    ...
    while (e == null) {
      ...
      if (e != null
          && e.t == EType.KILLSENDER) { // We need the possibility to cancel a get_ts in case we had a put_ts error.
        return null;
      }
      if (networkThread.getDone()) // dispatch loop has finished
      {
