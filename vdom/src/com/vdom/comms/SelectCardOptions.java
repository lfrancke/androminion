 ...
 *
 * This class gives information about the constrains set on the selection of cards, e.g. what type, cost, from which
 * place
 */
public class SelectCardOptions implements Serializable {
  ...
  public SelectCardOptions defaultCardSelected(int c) {
    defaultCardSelected = c;
    return this;
  }
  ...
  public SelectCardOptions setCount(int c) {
    count = c;
    return this;
  }

  public SelectCardOptions setMinCount(int c) {
    minCount = c;
    return this;
  }
  ...
  public SelectCardOptions minCost(int c) {
    minCost = c;
    return this;
  }

  public SelectCardOptions maxCost(int c) {
    maxCost = c;
    return this;
  }
  ...
  public SelectCardOptions exactCost(int coin, int debt, int potion) {
    ...
    minDebtCost = debt;
    maxDebtCost = debt;
    minPotionCost = potion;
    maxPotionCost = potion;
    return this;
  }

  public SelectCardOptions maxPotionCost(int c) {
    maxPotionCost = c;
    return this;
  }
  ...
  public SelectCardOptions maxDebtCost(int c) {
    maxDebtCost = c;
    return this;
  }

  public SelectCardOptions copperCountInPlay(int c) {
    copperCountInPlay = c;
    return this;
  }

  public SelectCardOptions not(Card c) {
    except = c;
    return this;
  }
  ...
  public SelectCardOptions applyOptionsToPile() {
    applyOptionsToPile = true;
    return this;
  }

  public SelectCardOptions token(PlayerSupplyToken c) {
    token = c;
    return this;
  }
  ...
  public boolean checkValid(Card c, int cost, boolean cardIsVictory, MoveContext context) {
    ...
    if (except != null && except.equals(c)) {
      return false;
    }
    ...
    if ((maxDebtCost >= 0) && (debtCost > maxDebtCost)) {
      return false;
    }
    if ((minDebtCost >= 0) && (debtCost < minDebtCost)) {
      return false;
    }
    if ((maxPotionCost >= 0) && (potionCost > maxPotionCost)) {
      return false;
    }
    if ((minPotionCost >= 0) && (potionCost < minPotionCost)) {
      return false;
    }
    if ((minDebtCost >= 0) && (debtCost < minDebtCost)) {
      return false;
    }
    if (lessThanMax && cost >= maxCost && debtCost >= maxDebtCost && potionCost >= maxPotionCost) {
      return false;
    }
    if (isReaction && !(c.is(Type.Reaction, p))) {
      return false;
    }
    if (isTreasure && !(c.is(Type.Treasure, p))) {
      return false;
    }
    if (isNonTreasure && (c.is(Type.Treasure, p))) {
      return false;
    }
    if (isVictory && !cardIsVictory) {
      return false;
    }
    if (isNonVictory && cardIsVictory) {
      return false;
    }
    if (fromPrizes && !c.is(Type.Prize, null)) {
      return false;
    }
    if (fromTable && !fromPrizes && c.is(Type.Prize, null)) {
      return false;
    }
    if (isNonRats && c.equals(Cards.rats)) {
      return false;
    }
    if (c.equals(Cards.grandMarket) && copperCountInPlay > 0) {
      return false;
    }
    if (isNonShelter && c.is(Type.Shelter, p)) {
      return false;
    }
    if (isAttack && !c.is(Type.Attack, p)) {
      return false;
    }
    if (isAction && !c.is(Type.Action, p)) {
      return false;
    }
    if (!isBuyPhase && c.is(Type.Event, null)) {
      return false;
    }
    if (isCastle && !c.is(Type.Castle, null)) {
      return false;
    }
    if (applyOptionsToPile && !c.isPlaceholderCard()) {
      return false;
    }
    if (!applyOptionsToPile && c.isPlaceholderCard()) {
      return false;
    ...
    if (isBuyPhase && !Cards.isSupplyCard(c) && !c.is(Type.Event, null)) {
      return false;
    }
    if (isSupplyCard && !Cards.isSupplyCard(c)) {
      return false;
    }
