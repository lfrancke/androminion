...
public class CardView extends FrameLayout implements OnLongClickListener, CheckableEx {
  ...
  public boolean onLongClick(View view) {
    ...
    if (f.exists() && (showimages.equals("Image") || showimages.equals("Both"))) {
      ...
      LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                                                                        (int) (im.getDrawable().getIntrinsicWidth()
                                                                               * zoom),
                                                                        (int) (im.getDrawable().getIntrinsicHeight()
                                                                               * zoom));
    ...
    if (wikilink) {
      ...
      text = "<a href=\"http://wiki.dominionstrategy.com/index.php/" + str2 + "\">" + view.getContext().getString(
    ...
    if (!isEnglish && PreferenceManager.getDefaultSharedPreferences(view.getContext())
                        .getBoolean("showenglishnames", false)) {
      title += " (" + englishName + ")";
  ...
  void setOnTable(boolean onTable) {
    ...
    if (cardDesc != null) {
      cardDesc.setVisibility(
        (onTable && ("viewstyle-descriptive".equals(viewstyle) || "viewstyle-condensed".equals(viewstyle))) ? VISIBLE
          : GONE);
    }
...
/*if (viewstyle.equals("viewstyle-simple"))*/
  ...
  private String getExtraDescription() {
    ...
    if (hasPlayerTokens || numEmbargos > 0 || numPileVpTokens > 0 || numPileDebtTokens > 0
      ...
      if (numEmbargos > 0) {
        text +=
          getContext().getString(R.string.token_embargo) + getContext().getString(R.string.token_colon) + numEmbargos;
        text += "\n";
      }
      if (numPileVpTokens > 0) {
        text += getContext().getString(R.string.token_victory) + getContext().getString(R.string.token_colon)
                + numPileVpTokens;
        text += "\n";
      }
      if (numPileDebtTokens > 0) {
        text += getContext().getString(R.string.token_debt) + getContext().getString(R.string.token_colon)
                + numPileDebtTokens;
        text += "\n";
      }
      if (numPileTradeRouteTokens > 0) {
        text += getContext().getString(R.string.token_trade_route) + getContext().getString(R.string.token_colon)
                + numPileTradeRouteTokens;
        text += "\n";
      }
      if (hasPlayerTokens) {
